...
public class ImportApplicationUser {
    ...
    public static void main(String[] args) {
        ...
        List<PartnerTableInfo> userInfoList =
                EasyExcel.read(fileName).head(PartnerTableInfo.class).sheet().doReadSync();
        System.out.println("总数 = "+ userInfoList.size());
        Map<String, List<PartnerTableInfo>> listMap =
                userInfoList.stream().
                        filter(userInfo -> StringUtils.isNotEmpty(userInfo.getUsername())).
                        collect(Collectors.groupingBy(PartnerTableInfo::getUsername));
